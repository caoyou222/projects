...
def main():
    ...
    #Test
    #.
    #img_test_batch, labels_test_batch = read_and_decode('tower-test-32.tfrecords',500)
    with tf.Session() as sess:
        path = "./turtle.png"
        src = cv2.imread(path)
        cv2.imshow("turtle ", src)
        ...
        saver.restore(sess, './stable_32/tower_model_stable.ckpt')
        ...
        print('The element is',np.argmax(result[0]))
        # coord.request_stop()
        # coord.join(threads)
